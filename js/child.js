...
$.getJSON("../json/child.json",function(data){
	...
	for(var i=0;i<20;i++){
		var $product_content='<div class="product_content">'+'<dl>'+'<dt class="shop_pic">'+'<img src="'+data.content.content_pic[i].src+'"/>'+'</dt>'+
						'<dd class="shop_name">'+data.content.con_name[i].name+'</dd>'+'<dd class="shop_price">'+'<strong>'+data.content.con_price[i].price+'</strong>'+'<i>￥197.00元</i>'+'</dd>'+
						'</dl>'+'<div class="shopping_part">'+'<p>评价：<b>0</b>人气：89'+'</p>'+
						'<input type="button" value="放入购物车" id="shoppingcart" />'+'<a href="javascript:;">收藏'+'</a>'+'</div>'+'</div>';
